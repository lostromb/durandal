...
namespace Durandal.Tests.TestSchemas
{
    ...
    {
        private static HashSet<string> _inheritance = new HashSet<string>() { "http://schema.org/Quantity", "http://schema.org/Intangible", "http://schema.org/Thing" };

        public Distance(KnowledgeContext context, string entityId = null) : base(context, "http://schema.org/Distance", entityId) { Initialize(); }

        /// <summary>
        /// Casting constructor
        /// </summary>
        /// <param name="castFrom">The entity that this one is being cast from</param>
        public Distance(Entity castFrom) : base(castFrom, "http://schema.org/Distance") { Initialize(); }
