		...
		protected void ScaleSampleOntoBuffer(byte[] originalData, int scaledLength)
		{
			...
			if (scaledLength > scaledBuffer.Length - sampleBufferWritePosition)
			{
				...
				else
				{
					...
					ArrayExtensions.MemMove(scaledBuffer, sampleBufferReadPosition, 0, sampleBufferWritePosition);
				}
			...
			{
                ArrayExtensions.MemCopy(originalData, 0, scaledBuffer, sampleBufferWritePosition, scaledLength);
