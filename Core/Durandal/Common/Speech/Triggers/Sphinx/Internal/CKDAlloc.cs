...
{
    internal static class CKDAlloc
    {
        internal static Pointer<T> ckd_malloc<T>(uint num_elements) where T : struct
        {
            return PointerHelpers.Malloc<T>(num_elements);
        }

        internal static Pointer<T> ckd_malloc<T>(int num_elements) where T : struct
        {
            return PointerHelpers.Malloc<T>(num_elements);
        }

        internal static Pointer<T> ckd_calloc<T>(int num_elements) where T : struct
        {
        ...
        internal static Pointer<T> ckd_calloc<T>(uint num_elements) where T : struct
        {
            // The semantics of this function rely on zero-initialization, which fortunately the runtime does for us
            return new Pointer<T>(new T[num_elements]);
        }

        internal static Pointer<T> ckd_calloc_struct<T>(int num_elements) where T : new()
        {
        ...
        internal static Pointer<T> ckd_calloc_struct<T>(uint num_elements) where T : new()
        {
            T[] array = new T[num_elements];
            for (int c = 0; c < num_elements; c++)
            {
        ...
        internal static Pointer<byte> ckd_salloc(Pointer<byte> orig)
        {
            uint len;
            Pointer<byte> buf;
        ...
        internal static Pointer<T> ckd_realloc<T>(Pointer<T> ptr, int new_size_in_elements)
        {
            return ptr.Realloc(new_size_in_elements);
        }

        internal static Pointer<T> ckd_realloc<T>(Pointer<T> ptr, uint new_size_in_elements)
        {
        ...
        /// <returns></returns>
        internal static Pointer<Pointer<T>> ckd_calloc_2d<T>(uint d1, uint d2) where T : struct
        {
        ...
        internal static Pointer<Pointer<T>> ckd_calloc_struct_2d<T>(uint d1, uint d2) where T : new()
        {
            Pointer<Pointer<T>> _ref;
            Pointer<T> mem;
            uint i, offset;
        ...
        internal static Pointer<Pointer<Pointer<T>>> ckd_calloc_3d<T>(uint d1, uint d2, uint d3) where T : struct
        {
        ...
        internal static Pointer<Pointer<Pointer<T>>> ckd_calloc_struct_3d<T>(uint d1, uint d2, uint d3) where T : new()
        {
            Pointer<Pointer<Pointer<T>>> ref1;
            Pointer<Pointer<T>> ref2;
            Pointer<T> mem;
        ...
        /// <returns></returns>
        internal static Pointer<Pointer<Pointer<T>>> ckd_alloc_3d_ptr<T>(
            uint d1,
           uint d2,
           uint d3,
