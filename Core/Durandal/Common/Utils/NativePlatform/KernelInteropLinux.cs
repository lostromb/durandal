...
namespace Durandal.Common.Utils.NativePlatform
{
    internal partial class KernelInteropLinux
    {
        ...
        internal static PlatformArchitecture TryGetArchForUnix(ILogger logger)
        {
            ...
            try
            {
                logger.Log("Running uname to determine system info...", LogLevel.Vrb);
                ...
                using (StreamReader stdOut = procInfo.StandardOutput)
                {
                    ...
                    {
                        logger.Log($"Got architecture from uname: {output}", LogLevel.Vrb);
                        return parsedArch;
                    }
                }
            }
