...
namespace Durandal.Common.Audio.Codecs.Opus.Ogg
{
    internal partial class StreamReadBuffer : IDisposable
    {
        ...
        public int MaxSize
        {
            ...
            set
            {
                ...
                if (newMaxSize < _end)
                {
                    ...
                    ArrayExtensions.MemCopy(_data, 0, newBuf, 0, _end);
                    _data = newBuf;
                }
                _maxSize = newMaxSize;
            }
        ...
        public int Read(long offset, byte[] buffer, int index, int count)
        {
            ...
            ArrayExtensions.MemCopy(_data, startIdx, buffer, index, count);
        ...
        void EnsureBufferSize(int reqSize, bool copyContents, int copyOffset)
        {
            ...
            if (copyContents)
            {
                ...
                {
                    // copy forward
                    ArrayExtensions.MemCopy(_data, copyOffset, newBuf, 0, _end - copyOffset);
                ...
                else if (copyOffset < 0 && -copyOffset < _end)
                {
                    ...
                    {
                        ArrayExtensions.MemCopy(_data, 0, newBuf, -copyOffset, Math.Max(_end, Math.Min(_end, _data.Length + copyOffset)));
                    }
                    else
                    {
        ...
        void CommitDiscard()
        {
            ...
            else
            {
                ...
                ArrayExtensions.MemCopy(_data, _discardCount, _data, 0, _end - _discardCount);
                _baseOffset += _discardCount;
                _end -= _discardCount;
            }
            _discardCount = 0;
