...
{
    /// <summary>
    /// Variant of <see cref="ManagedCRC32C"/> which runs on big-endian platforms.
    /// </summary>
    internal class ManagedCRC32C_BE : ICRC32C
    {
        /// <inheritdoc />
        ...
        public void Slurp(ref CRC32CState state, ReadOnlySpan<byte> input_8)
        {
            ...
            {
                throw new PlatformNotSupportedException("Cannot use big-endian CRC algorithm on a little-endian platform");
            }
#endif
            uint crc = state.Checksum ^ 0xffffffff;
            ...
            int offset = BinaryHelpers.GetMemoryAlignment(input_8, sizeof(uint));
            ...
            {
                crc ^= BinaryHelpers.ReverseEndianness(input_32[input_idx_32]);
                uint high = BinaryHelpers.ReverseEndianness(input_32[input_idx_32 + 1]);
                uint high2 = BinaryHelpers.ReverseEndianness(input_32[input_idx_32 + 2]);
