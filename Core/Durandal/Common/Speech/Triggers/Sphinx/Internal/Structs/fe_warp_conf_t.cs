...
namespace Durandal.Common.Speech.Triggers.Sphinx.Internal.Structs
{
    ...
    internal class fe_warp_conf_t
    {
        ...
        public delegate void set_parameters_func(Pointer<byte> param_str, float sampling_rate, SphinxLogger logger);
        public delegate Pointer<byte> doc_func();
        public delegate uint id_func();
        public delegate uint n_param_func();
        public delegate float warped_to_unwarped_func(float nonlinear, SphinxLogger logger);
        public delegate float unwarped_to_warped_func(float linear);
        public delegate void print_func(Pointer<byte> label, SphinxLogger logger);
    }
}
