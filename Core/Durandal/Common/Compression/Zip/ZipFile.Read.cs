...
namespace Durandal.Common.Compression.Zip
{
    ...
    public partial class ZipFile
    {
        ...
        ///
        public static ZipFile Read(VirtualPath fileName,
            IFileSystem fileSystem,
            ILogger logger)
        {
            return ZipFile.Read(fileName, fileSystem, logger, null, null, null);
        }
        ...
        /// <returns>The instance read from the zip archive.</returns>
        ///
        private static ZipFile Read(VirtualPath fileName,
            IFileSystem fileSystem,
            ILogger logger,
        ...
        {
            ZipFile zf = new ZipFile(logger, fileSystem);
        ...
        /// <returns>The ZipFile instance read from the stream.</returns>
        ///
        public static ZipFile Read(Stream zipStream, IFileSystem fileSystem,
            ILogger logger, ReadOptions options)
        {
            if (options == null)
                throw new ArgumentNullException("options");
        ...
        private static void ReadCentralDirectory(ZipFile zf)
        {
            ...
            while ((de = ZipEntry.ReadDirEntry(zf, zf._logger, zf._fileSystem, previouslySeen)) != null)
            {
                de.ResetDirEntry();
        ...
        private static void ReadIntoInstance_Orig(ZipFile zf)
        {
            ...
            while ((e = ZipEntry.ReadEntry(zc, zf._logger, zf._fileSystem, firstEntry)) != null)
            {
                if (zf.Verbose)
        ...
        ///
        /// <para>
        ///   Calling this method is equivalent to calling IsZipFile() with the testExtract parameter set to false.
        /// </para>
        /// </remarks>
        ///
        ...
        public static bool IsZipFile(VirtualPath fileName, IFileSystem fileSystem)
        {
            return IsZipFile(fileName, fileSystem, false);
        }
        ...
        public static bool IsZipFile(VirtualPath fileName, IFileSystem fileSystem, bool testExtract)
        {
            bool result = false;
            try
            {
                ...
                using (var s = fileSystem.OpenStream(fileName, FileOpenMode.Open, FileAccessMode.Read))
                {
