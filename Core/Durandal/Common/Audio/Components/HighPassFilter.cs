...
namespace Durandal.Common.Audio.Components
{
    public sealed class HighPassFilter : BiquadFilter
    {
        ...
        /// <summary>
        /// Constructs a new highpass filter.
        /// </summary>
        ...
        public HighPassFilter(WeakPointer<IAudioGraph> graph, AudioSampleFormat format, string nodeCustomName, float frequencyHz, float q = 0.70721f)
            : base(graph, format, nameof(HighPassFilter), nodeCustomName)
        {
            if (frequencyHz < 0)
            {
