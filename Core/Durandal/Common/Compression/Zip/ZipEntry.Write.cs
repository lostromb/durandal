...
namespace Durandal.Common.Compression.Zip
{
    public partial class ZipEntry
    {
        internal void WriteCentralDirectoryEntry(Stream s)
        {
            ...
            if (this.CompressionMethod == Durandal.Common.Compression.Zip.CompressionMethod.BZip2)
            ...
            // actual filename
            ArrayExtensions.MemCopy(fileNameBytes, 0, bytes, i, filenameLength);
            i += filenameLength;
            ...
            if (_Extra != null)
            {
                ...
                byte[] h = _Extra;
                int offx = 0;
                ArrayExtensions.MemCopy(h, offx, bytes, i, extraFieldLength);
                i += extraFieldLength;
            }
            ...
            if (commentLength != 0)
            {
                ...
                ArrayExtensions.MemCopy(_CommentBytes, 0, bytes, i, commentLength);
        ...
        private byte[] ConstructExtraField(bool forCentralDirectory)
        {
                 ...
                 (!forCentralDirectory || _entryRequiresZip64.Value)))
            {
                ...
                // uncompressed size
                ArrayExtensions.MemCopy(BitConverter.GetBytes(_UncompressedSize), 0, block, i, 8);
                i += 8;
                // compressed size
                ArrayExtensions.MemCopy(BitConverter.GetBytes(_CompressedSize), 0, block, i, 8);
                i += 8;
                ...
                {
                    // relative offset
                    ArrayExtensions.MemCopy(BitConverter.GetBytes(_RelativeOffsetOfLocalHeader), 0, block, i, 8);
                    i += 8;

                    // starting disk number
                    ArrayExtensions.MemCopy(BitConverter.GetBytes(0), 0, block, i, 4);
                }
            ...
            if (_ntfsTimesAreSet && _emitNtfsTimes)
            {
                ...
                ArrayExtensions.MemCopy(BitConverter.GetBytes(z), 0, block, i, 8);
                i += 8;
                z = _Atime.ToFileTime();
                ArrayExtensions.MemCopy(BitConverter.GetBytes(z), 0, block, i, 8);
                i += 8;
            ...
            if (_ntfsTimesAreSet && _emitUnixTimes)
            {
                ...
                ArrayExtensions.MemCopy(BitConverter.GetBytes(z), 0, block, i, 4);
                i += 4;
                if (!forCentralDirectory)
                {
            ...
            if (listOfBlocks.Count > 0)
            {
                ...
                for (i = 0; i < listOfBlocks.Count; i++)
                {
                    ArrayExtensions.MemCopy(listOfBlocks[i], 0, aggregateBlock, current, listOfBlocks[i].Length);
        ...
        private bool WantReadAgain()
        {
            ...
            if (CompressionLevel == Durandal.Common.Compression.ZLib.CompressionLevel.None) return false;
        ...
        private void MaybeUnsetCompressionMethodForWriting(int cycle)
        {
            ...
            else if ((this._Source == ZipEntrySource.FileSystem) && (SharedUtilities.GetFileLength(LocalFileName, _fileSystem) == 0L))
            {
                _CompressionMethod = 0x00;
                return;
            }
            ...
            if (CompressionLevel == (short)Durandal.Common.Compression.ZLib.CompressionLevel.None &&
                CompressionMethod == Durandal.Common.Compression.Zip.CompressionMethod.Deflate)
                _CompressionMethod = 0x00;
        ...
        internal void WriteHeader(Stream s, int cycle)
        {
            ...
            if (string.Equals(_actualEncoding.WebName, System.Text.Encoding.UTF8.WebName) ||
                _actualEncoding.WebName == "utf-8")
                _BitField |= 0x0800;
            ...
            {
                _TimeBlob = Durandal.Common.Compression.Zip.SharedUtilities.DateTimeToPacked(LastModified);
            }
            ...
            ArrayExtensions.MemCopy(block, 0, bytes, 0, i);
            //for (j = 0; j < i; j++) bytes[j] = block[j];

            // The filename written to the archive.
            ArrayExtensions.MemCopy(fileNameBytes, 0, bytes, i, fileNameBytes.Length);
            ...
            if (_Extra != null)
            {
                ArrayExtensions.MemCopy(_Extra, 0, bytes, i, _Extra.Length);
        ...
        Int32 FigureCrc32()
        {
            if (_crcCalculated == false)
            {
                ...
                {
                    var output = new Durandal.Common.Compression.CrcCalculatorStream(Stream.Null);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        input = _fileSystem.OpenStream(LocalFileName, FileOpenMode.Open, FileAccessMode.Read);
                    }

                    var crc32 = new Durandal.Common.Compression.PkZipCRC32();
...
#pragma warning disable CA2000 // Dispose objects before losing scope
                ...
                var output = new Durandal.Common.Compression.CrcCalculatorStream(compressor, true);
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        internal void FinishOutputStream(Stream s,
                                         ...
                                         Stream encryptor,
                                         Stream compressor,
                                         Durandal.Common.Compression.CrcCalculatorStream output)
        {
            ...
            if ((compressor as Durandal.Common.Compression.ZLib.DeflateStream) != null)
                compressor.Dispose();
            else if ((compressor as Durandal.Common.Compression.BZip2.BZip2OutputStream) != null)
                compressor.Dispose();
        ...
        internal void PostProcessOutput(Stream s)
        {
            ...
            if (_UncompressedSize == 0 && _CompressedSize == 0)
            {
                ...
                if (_Password != null)
                {
                    ...
                    if (Encryption != EncryptionAlgorithm.None)
                    {
                        ...
                        // workitem 10178
                        Durandal.Common.Compression.Zip.SharedUtilities.Workaround_Ladybug318918(s);
            ...
            if (_OutputUsesZip64.Value)
            {
                ...
                ArrayExtensions.MemCopy(BitConverter.GetBytes(_UncompressedSize), 0, _EntryHeader, i, 8);
                i += 8;
                ArrayExtensions.MemCopy(BitConverter.GetBytes(_CompressedSize), 0, _EntryHeader, i, 8);
            }
            else
            {
                 ...
                 (this._Source == ZipEntrySource.ZipOutputStream && s.CanSeek))
            {
                ...
                if (zss != null && _diskNumber != zss.CurrentSegment)
                {
                    ...
                    using (Stream hseg = ZipSegmentedStream.ForUpdate(this._container.ZipFile.Name, _fileSystem, _diskNumber))
                    {
            ...
            if (((_BitField & 0x0008) == 0x0008) && !IsDirectory)
            {
                ...
                // signature
                ArrayExtensions.MemCopy(BitConverter.GetBytes(ZipConstants.ZipEntryDataDescriptorSignature), 0, Descriptor, i, 4);
                i += 4;

                // CRC - the correct value now
                ArrayExtensions.MemCopy(BitConverter.GetBytes(_Crc32), 0, Descriptor, i, 4);
                i += 4;
                ...
                {
                    // CompressedSize - the correct value now
                    ArrayExtensions.MemCopy(BitConverter.GetBytes(_CompressedSize), 0, Descriptor, i, 8);
                    i += 8;

                    // UncompressedSize - the correct value now
                    ArrayExtensions.MemCopy(BitConverter.GetBytes(_UncompressedSize), 0, Descriptor, i, 8);
                    i += 8;
                }
                else
                {
        ...
        internal void PrepOutputStream(Stream s,
                                       ...
                                       out Durandal.Common.Compression.CrcCalculatorStream output)
        {
            ...
            // Wrap a CrcCalculatorStream around that.
            // This will happen BEFORE compression (if any) as we write data out.
            output = new Durandal.Common.Compression.CrcCalculatorStream(compressor, true);
        }
        ...
        {
            if (_CompressionMethod == 0x08 && CompressionLevel != Durandal.Common.Compression.ZLib.CompressionLevel.None)
            {
                var o = new Durandal.Common.Compression.ZLib.DeflateStream(s, Durandal.Common.Compression.ZLib.CompressionMode.Compress,
                                                     CompressionLevel,
                                                     true);
            ...
            {
                var o = new Durandal.Common.Compression.BZip2.BZip2OutputStream(s, true, _logger);
                return o;
            }
        ...
        private void CopyThroughWithRecompute(Stream outstream)
        {
            ...
            using (CountingStream input = new CountingStream(this.ArchiveStream))
            {
        ...
        {
            _logger.Log(string.Format("ZipEntry.Write " + format, varParams), LogLevel.Vrb);
        }
    }
}
