namespace Durandal.Tests.Common.Dialog.Runtime
{
    ...
    public class AppDomainContainerTests
    {
        ...
        private static AppDomainIsolatedPluginProvider _mockPluginProvider;
        ...
        public static void InitializeClass(TestContext context)
        {
            ...
            _mockPluginProvider = AppDomainIsolatedPluginProvider.Create(
        ...
        public static void CleanupClass()
        {
            ...
            _mockPluginProvider?.Dispose();
            _baseConfig?.Dispose();
            _conversationStateCache?.Dispose();
        ...
        [TestMethod]
        [Ignore]
        public async Task TestAppDomainContainerPluginBasicSuccess()
        {
            if (!_mockPluginProvider.HasDevRuntimeAvailable("netframework"))
            {
        ...
        [TestMethod]
        [Ignore]
        public async Task TestAppDomainContainerPluginOAuthService()
        {
            if (!_mockPluginProvider.HasDevRuntimeAvailable("netframework"))
            {
                Assert.Inconclusive("Dev runtime not found for process-level container. Will not run test in case the runtime version is out of date.");
        ...
        [TestMethod]
        [Ignore]
        public async Task TestAppDomainContainerPluginViewData()
        {
            if (!_mockPluginProvider.HasDevRuntimeAvailable("netframework"))
            {
                Assert.Inconclusive("Dev runtime not found for process-level container. Will not run test in case the runtime version is out of date.");
        ...
        /// FileNotFoundException: Could not load file or assembly 'Durandal, Version=21.0.4486.0, Culture=neutral, PublicKeyToken=2f7fb9b49ffdfe20' or one of its dependencies. The system cannot find the file specified.
        /// [14:44:58]
        /// [E: ContainerHost-UnitTestPlugin]
        /// Server stack trace: 
        /// at System.Reflection.RuntimeAssembly.GetExportedTypes(RuntimeAssembly assembly, ObjectHandleOnStack retTypes)
        /// at System.Reflection.RuntimeAssembly.GetExportedTypes()
        /// at Durandal.Common.Dialog.Runtime.ResidentDllPluginLoader.RefreshPluginDirectoryIfNeeded() in C:\Code\Durandal\Core\Durandal.NonPortable\Dialog\Runtime\ResidentDllPluginLoader.cs:line 481
        /// at Durandal.Common.Dialog.Runtime.ResidentDllPluginLoader..ctor(IDialogExecutor executor, ILogger logger, IFileSystem pluginFileSystem, VirtualPath pluginDirectory, IFileSystem pluginContentFileSystem, PluginFrameworkLevel maxPluginFrameworkLevel, Boolean doShadowCopy) in C:\Code\Durandal\Core\Durandal.NonPortable\Dialog\Runtime\ResidentDllPluginLoader.cs:line 79
        /// at Durandal.Common.Remoting.AppDomainContainerGuest.Initialize(String serializedJsonInitializationParameters) in C:\Code\Durandal\Core\Durandal.NetFramework\Remoting\AppDomainContainerGuest.cs:line 171
        /// </summary>
        [TestMethod]
        [Ignore]
        public async Task TestAppDomainContainerKeepAlive()
        {
            if (!_mockPluginProvider.HasDevRuntimeAvailable("netframework"))
            {
                Assert.Inconclusive("Dev runtime not found for process-level container. Will not run test in case the runtime version is out of date.");
        ...
        [TestMethod]
        [Ignore]
        public async Task TestAppDomainContainerRecoversAfterCrash()
        {
            if (!_mockPluginProvider.HasDevRuntimeAvailable("netframework"))
            {
                Assert.Inconclusive("Dev runtime not found for process-level container. Will not run test in case the runtime version is out of date.");
