...
{
    /// <summary>
    /// A double-ended queue (deque), which provides O(1) indexed access, O(1) removals from the front and back, amortized O(1) insertions to the front and back, and O(N) insertions
    /// and removals anywhere else (with the operations getting slower as the index approaches the middle).
    /// </summary>
    ...
    public sealed class Deque<T> : IList<T>, IReadOnlyList<T>, System.Collections.IList
    {
        ...
        /// <exception cref="System.ArgumentOutOfRangeException"><paramref name="index"/> is not a valid index in this list.</exception>
        /// <exception cref="System.NotSupportedException">This property is set and the list is read-only.</exception>
        ...
        /// <summary>
        /// Copies the elements of this list to an <see cref="System.Array"/>, starting at a particular <see cref="System.Array"/> index.
        /// </summary>
        /// <param name="array">The one-dimensional <see cref="System.Array"/> that is the destination of the elements copied from this slice. The <see cref="System.Array"/> must have zero-based indexing.</param>
        ...
        private void CopyToArray(T[] array, int arrayIndex = 0)
        {
            if (array == null)
            {
                throw new ArgumentNullException("array");
            ...
            if (IsSplit)
            {
                ...
                ArrayExtensions.MemCopy(_buffer, _offset, array, arrayIndex, length);
                ArrayExtensions.MemCopy(_buffer, 0, array, arrayIndex + length, Count - length);
            }
            else
            {
                // The existing buffer is whole
                ArrayExtensions.MemCopy(_buffer, _offset, array, arrayIndex, Count);
            }
        }
