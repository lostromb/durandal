...
namespace Durandal.Common.Compression.Zip
{
    ...
    {
        private VirtualPath ArchiveNameForEvent
        {
            get
            {
                return (_name != null) ? _name : new VirtualPath("\\stream");
            }
        }
        ...
        /// </example>
        ///
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.ReadProgress"/>
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.AddProgress"/>
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.ExtractProgress"/>
        ...
        /// </remarks>
        ///
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.SaveProgress"/>
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.AddProgress"/>
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.ExtractProgress"/>
            ...
            get
            {
                if (_lengthOfReadStream == -99)
                {
                    _lengthOfReadStream = (_ReadStreamIsOurs)
                        ? SharedUtilities.GetFileLength(_name, _fileSystem)
                        : -1L;
                }
        ...
        private void OnExtractEntry(int current, bool before, ZipEntry currentEntry, VirtualPath path)
        {
        ...
        internal bool OnSingleEntryExtract(ZipEntry entry, VirtualPath path, bool before)
        {
            EventHandler<ExtractProgressEventArgs> ep = ExtractProgress;
            if (ep != null)
            {
        ...
        internal bool OnExtractExisting(ZipEntry entry, VirtualPath path)
        {
            EventHandler<ExtractProgressEventArgs> ep = ExtractProgress;
            if (ep != null)
            {
        ...
        private void OnExtractAllCompleted(VirtualPath path)
        {
            EventHandler<ExtractProgressEventArgs> ep = ExtractProgress;
            if (ep != null)
            {
        ...
        private void OnExtractAllStarted(VirtualPath path)
        {
            EventHandler<ExtractProgressEventArgs> ep = ExtractProgress;
            if (ep != null)
            {
        ...
        ///     time.  For example, when calling AddDirectory() on a
        ...
        ///
        /// <code lang="VB">
        /// Private Sub MyZipError(ByVal sender As Object, ByVal e As Durandal.Common.Compression.Zip.ZipErrorEventArgs)
