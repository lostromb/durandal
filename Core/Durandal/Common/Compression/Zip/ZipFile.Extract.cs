...
namespace Durandal.Common.Compression.Zip
{
    ...
    public partial class ZipFile
    {
        ...
        ///   ZipFile.ExtractAll() overload that allows you to
        ...
        /// </param>
        ///
        public void ExtractAll(VirtualPath path)
        {
        ...
        /// The action to take when an extract would overwrite an existing file
        /// applies to all entries.  If you want to set this on a per-entry basis,
        /// then you must use ZipEntry.Extract() or one of the similar methods.
        /// </para>
        ///
        /// <para>
        /// Calling this method is equivalent to setting the <see
        /// cref="ExtractExistingFile"/> property and then calling ExtractAll().
        /// </para>
        ///
        /// <para>
        ...
        /// </param>
        public void ExtractAll(VirtualPath path, ExtractExistingFileAction extractExistingFile)
        {
        ...
        private void _InternalExtractAll(VirtualPath path, bool overrideExtractExistingProperty)
        {
            bool header = Verbose;
            _inExtractAll = true;
            try
            {
                ...
                if (!_extractOperationCanceled)
                {
                    ...
                    foreach (ZipEntry e in _entries.Values)
                    {
                        ...
                        {
                            VirtualPath outputFile = (e.FileName.StartsWith("/"))
                                ? path.Combine(e.FileName.Substring(1))
                                : path.Combine(e.FileName);
