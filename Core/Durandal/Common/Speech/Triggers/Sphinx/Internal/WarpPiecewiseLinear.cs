...
{
    internal static class WarpPiecewiseLinear
    {
        public const int N_PARAM = 2;
        internal static Pointer<float> parameters = new Pointer<float>(new float[] { 1.0f, 6800.0f });
        internal static Pointer<float> final_piece = PointerHelpers.Malloc<float>(2);
        ...
        internal static Pointer<byte> fe_warp_piecewise_linear_doc()
        {
            return cstring.ToCString("piecewise_linear :== < w' = a * w, w < F >");
        }

        internal static uint fe_warp_piecewise_linear_id()
        {
            return Frontend.FE_WARP_ID_PIECEWISE_LINEAR;
        }

        internal static uint fe_warp_piecewise_linear_n_param()
        {
            return N_PARAM;
        }

        internal static void fe_warp_piecewise_linear_set_parameters(Pointer<byte> param_str,
                                        float sampling_rate, SphinxLogger logger)
        {
            Pointer<byte> tok;
        ...
        internal static float fe_warp_piecewise_linear_warped_to_unwarped(float nonlinear, SphinxLogger logger)
        {
            if (is_neutral != 0)
            {
                return nonlinear;
        ...
        internal static float fe_warp_piecewise_linear_unwarped_to_warped(float linear)
        {
            if (is_neutral != 0)
            {
                return linear;
        ...
        internal static void fe_warp_piecewise_linear_print(Pointer<byte> label, SphinxLogger logger)
        {
            uint i;
