...
namespace Durandal.Common.Compression.Zip
{
    internal class ZipSegmentedStream : System.IO.Stream
    {
        ...
        private VirtualPath _baseName;
        private VirtualPath _baseDir;
        private IFileSystem _fileSystem;
        //private bool _isDisposed;
        private VirtualPath _currentName;
        private VirtualPath _currentTempName;
        ...
        public static ZipSegmentedStream ForReading(VirtualPath name,
        ...
        public static ZipSegmentedStream ForWriting(VirtualPath name, IFileSystem fileSystem, ILogger logger, int maxSegmentSize)
        {
            ZipSegmentedStream zss = new ZipSegmentedStream()
                {
                    rwMode = RwMode.Write,
        ...
        ///   </para>
        /// </remarks>
        public static Stream ForUpdate(VirtualPath name, IFileSystem fileSystem, uint diskNumber)
        {
            //VirtualPath fname =
            ...
            VirtualPath fname = name.Container.Combine(name.NameWithoutExtension +
                                        string.Format(".z{0:D2}", diskNumber + 1));
            ...
            return fileSystem.OpenStream(fname, FileOpenMode.Open, FileAccessMode.ReadWrite);
        }
        ...
        ///   </para>
        /// </remarks>
        public VirtualPath CurrentName
        {
            get
            {
        ...
        public VirtualPath CurrentTempName
        {
            get
            {
        ...
        private VirtualPath _NameForSegment(uint diskNumber)
        {
            return _baseName.Container.Combine(_baseName.NameWithoutExtension +
                                        string.Format(".z{0:D2}", diskNumber + 1));
        }
        ...
        private void _SetReadStream()
        {
            ...
            _innerStream = _fileSystem.OpenStream(CurrentName, FileOpenMode.Open, FileAccessMode.Read);
        }
        ...
        private void _SetWriteStream(uint increment)
        {
            ...
            {
                _innerStream.Dispose();
                if (_fileSystem.Exists(CurrentName))
                    _fileSystem.Delete(CurrentName);
                FileHelpers.MoveFile(_fileSystem, _currentTempName, _fileSystem, CurrentName, _logger);
            ...
            SharedUtilities.CreateAndOpenUniqueTempFile(_baseDir, _fileSystem,
                                                        out _innerStream,
        ...
        public long TruncateBackward(uint diskNumber, long offset)
        {
            ...
            {
                _innerStream.Dispose();
                if (_fileSystem.Exists(_currentTempName))
                    _fileSystem.Delete(_currentTempName);
            }

            // Now, remove intervening segments.
            for (uint j= CurrentSegment-1; j > diskNumber; j--)
            {
                VirtualPath s = _NameForSegment(j);
                // Console.WriteLine("***ZSS.Trunc:  removing file {0}", s);
                if (_fileSystem.Exists(s))
                    _fileSystem.Delete(s);
            }
            ...
            for (int i = 0; i < 3; i++)
            {
                try
                {
                    _currentTempName = CurrentName.Container.Combine(SharedUtilities.InternalGetTempFileName());
                    // move the .z0x file back to a temp name
                    FileHelpers.MoveFile(_fileSystem, CurrentName, _fileSystem, _currentTempName, _logger);
            ...
            // open it
            _innerStream = _fileSystem.OpenStream(_currentTempName, FileOpenMode.OpenOrCreate, FileAccessMode.ReadWrite);
