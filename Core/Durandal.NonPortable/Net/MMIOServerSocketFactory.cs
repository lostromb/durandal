...
{
    /// <summary>
    /// <see cref="IServerSocketFactory"/> implementation which returns <see cref="MMIOServerSocket"/>.
    /// </summary>
    public class MMIOServerSocketFactory : IServerSocketFactory
    {
        ...
        public ISocket CreateServerSocket(ILogger logger, WeakPointer<IMetricCollector> metrics, DimensionSet metricDimensions, int minimumBufferSizeBytes = -1)
        {
            ...
            return new MMIOServerSocket(logger.Clone("MMIOServer"), metrics, metricDimensions, minimumBufferSizeBytes);
        }
    }
}
