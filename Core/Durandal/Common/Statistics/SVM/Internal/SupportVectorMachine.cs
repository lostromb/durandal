...
namespace Durandal.Common.Statistics.SVM
{
    public static class SupportVectorMachine
    {
        ...
        public static readonly IRandom rand = new FastRandom();
                        ...
                        ILogger logger)
        {
            ...
            if (Cp == Cn)
                logger.Log("nu = " + sum_alpha / (Cp * prob.ProblemSize), LogLevel.Vrb);
        ...
        private static void solve_nu_svc(SVMProblem prob, SVMParam param,
                        double[] alpha, Solver.SolutionInfo si, ILogger logger)
        {
            int i;
            int l = prob.ProblemSize;
            double nu = param.nu;
                        ...
                        double[] alpha, Solver.SolutionInfo si, ILogger logger)
        {
            ...
            logger.Log("nu = " + sum_alpha / (param.C * l), LogLevel.Vrb);
        }
                        ...
                        double[] alpha, Solver.SolutionInfo si, ILogger logger)
        {
            ...
            logger.Log("epsilon = " + (-si.r), LogLevel.Vrb);
        ...
        static decision_function svm_train_one(
            ...
            double Cp, double Cn, ILogger logger)
        {
            ...
            switch (param.SVMType)
            {
                case SVMParam.SVM_TYPE_C_SVC:
                    solve_c_svc(prob, param, alpha, si, Cp, Cn, logger);
                    break;
                case SVMParam.SVM_TYPE_NU_SVC:
                    solve_nu_svc(prob, param, alpha, si, logger);
                    break;
                case SVMParam.SVM_TYPE_ONE_CLASS:
                    solve_one_class(prob, param, alpha, si);
                    break;
                case SVMParam.SVM_TYPE_EPSILON_SVR:
                    solve_epsilon_svr(prob, param, alpha, si, logger);
                    break;
                case SVMParam.SVM_TYPE_NU_SVR:
                    solve_nu_svr(prob, param, alpha, si, logger);
                    break;
            }

            logger.Log("obj = " + si.Obj + ", rho = " + si.Rho, LogLevel.Vrb);
                      ...
                      double[] probAB, ILogger logger)
        {
            ...
            for (iter = 0; iter < max_iter; iter++)
            {
                ...
                {
                    logger.Log("Line search fails in two-class probability estimates", LogLevel.Vrb);
                    break;
                }
            }

            if (iter >= max_iter)
                logger.Log("Reaching maximal iterations in two-class probability estimates", LogLevel.Vrb);
        ...
        private static void multiclass_probability(int k, double[][] r, double[] p, ILogger logger)
        {
            int t, j;
            ...
            if (iter >= max_iter)
                logger.Log("Exceeds max_iter in multiclass_prob", LogLevel.Vrb);
        }

        // Cross-validation decision values for probability estimates
        private static void svm_binary_svc_probability(SVMProblem prob, SVMParam param, double Cp, double Cn, double[] probAB, Func<int, bool> shouldTerminate, ILogger logger)
        {
            int i;
            int nr_fold = 5;
            ...
            {
                int j = i + rand.NextInt(prob.ProblemSize - i);
            ...
            for (i = 0; i < nr_fold; i++)
            {
                ...
                else
                {
                    ...
                    SVMModel submodel = svm_train(subprob, subparam, shouldTerminate, logger);
                    for (j = begin; j < end; j++)
                    {
            ...
            sigmoid_train(prob.ProblemSize, dec_values, prob.y, probAB, logger);
        }

        // Return parameter of a Laplace distribution 
        private static double svm_svr_probability(SVMProblem prob, SVMParam param, Func<int, bool> shouldTerminate, ILogger logger)
        {
            int i;
            int nr_fold = 5;
            ...
            svm_cross_validation(prob, newparam, nr_fold, ymv, shouldTerminate, logger);
            for (i = 0; i < prob.ProblemSize; i++)
            {
            ...
            logger.Log("Prob. model for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma=" + mae + "\n");
            return mae;
        }
        ...
        // Interface functions
        //
        public static SVMModel svm_train(SVMProblem prob, SVMParam param, Func<int, bool> shouldTerminate, ILogger logger)
        {
               ...
               param.SVMType == SVMParam.SVM_TYPE_NU_SVR)
            {
                    ...
                    param.SVMType == SVMParam.SVM_TYPE_NU_SVR))
                {
                    ...
                    model.ProbA[0] = svm_svr_probability(prob, param, shouldTerminate, logger);
                }

                decision_function f = svm_train_one(prob, param, 0, 0, logger);
            ...
            else
            {
                ...
                if (nr_class == 1)
                    logger.Log("WARNING: training data in only one class. See README for details.\n");
                ...
                for (i = 0; i < param.NumberWeight; i++)
                {
                    ...
                    for (j = 0; j < nr_class; j++)
                        if (param.WeightLabel[i] == label[j])
                            break;
                    if (j == nr_class)
                        logger.Log("Class label " + param.WeightLabel[i] + " specified in weight is not found", LogLevel.Wrn);
                    else
                ...
                for (i = 0; i < nr_class; i++)
                    for (int j = i + 1; j < nr_class; j++)
                    {
                        ...
                        if (param.DoProbabilityEstimate)
                        {
                            ...
                            svm_binary_svc_probability(sub_prob, param, weighted_C[i], weighted_C[j], probAB, shouldTerminate, logger);
                            probA[p] = probAB[0];
                            probB[p] = probAB[1];
                        }

                        f[p] = svm_train_one(sub_prob, param, weighted_C[i], weighted_C[j], logger);
                        for (k = 0; k < ci; k++)
        ...
        public static void svm_cross_validation(SVMProblem prob, SVMParam param, int nr_fold, double[] target, Func<int, bool> shouldTerminate, ILogger logger)
        {
            int i;
            ...
            for (i = 0; i < nr_fold; i++)
            {
                ...
                SVMModel submodel = svm_train(subprob, param, shouldTerminate, logger);
                if (param.DoProbabilityEstimate &&
                    ...
                    param.SVMType == SVMParam.SVM_TYPE_NU_SVC))
                {
                    ...
                    for (j = begin; j < end; j++)
                        target[perm[j]] = svm_predict_probability(submodel, prob.x[perm[j]], prob_estimates, logger);
                }
                else
                    for (j = begin; j < end; j++)
        ...
        public static double svm_get_svr_probability(SVMModel model, ILogger logger)
        {
            ...
            {
                logger.Log("Model doesn't contain information for SVR probability inference\n");
                return 0;
            }
        }
        ...
        public static double svm_predict_probability(SVMModel model, SVMNode[] x, double[] prob_estimates, ILogger logger)
        {
                ...
                model.ProbA != null && model.ProbB != null)
            {
                ...
                multiclass_probability(nr_class, pairwise_prob, prob_estimates, logger);
