...
namespace Durandal.Common.Compression.Zip
{
    ...
    public partial class ZipFile
    {
        ...
        private void DeleteFileWithRetry(VirtualPath filename)
        {
            bool done = false;
            int nRetries = 3;
            for (int i=0; i < nRetries && !done; i++)
            {
                ...
                {
                    _logger.Log("************************************************** Retry delete.", Logger.LogLevel.Wrn);
                    Task.Delay(200 + i * 200).Await();
                }
            }
        }
        ...
        ///
        /// <exception cref="Durandal.Common.Compression.Zip.BadStateException">
        ...
        public void Save()
        {
            try
            {
                ...
                if (_name != null && string.Equals(_name.Extension, ".exe", StringComparison.OrdinalIgnoreCase) && !_SavingSfx)
                    ...
                    (_temporaryFileName!=null || zss != null))
                {
                    ...
                    VirtualPath tmpName = null;
                    if (_fileSystem.Exists(_name))
                    {
                        // the steps:
                        //
                        // 1. Delete tmpName
                        ...
                        if (_fileSystem.Exists(tmpName))
                            DeleteFileWithRetry(tmpName);
                        FileHelpers.MoveFile(_fileSystem, _name, _fileSystem, tmpName, _logger);
                    }

                    OnSaveEvent(ZipProgressEventType.Saving_BeforeRenameTempArchive);
                    VirtualPath src = (zss != null) ? zss.CurrentTempName : _temporaryFileName;
                    FileHelpers.MoveFile(_fileSystem, src, _fileSystem, _name, _logger);
        ...
        private void RemoveTempFile()
        {
            try
            {
                if (_fileSystem.Exists(_temporaryFileName))
                {
                    _fileSystem.Delete(_temporaryFileName);
                }
            }
        ...
        ///
        /// </example>
        public void Save(VirtualPath fileName)
        {
            ...
            _name = fileName;
            if (_fileSystem.Exists(_name) && _fileSystem.WhatIs(_name) == ResourceType.Directory)
            ...
            _fileAlreadyExists = _fileSystem.Exists(_readName);
            Save();
        }
        ...
        public void Save(Stream outputStream)
        {
            ...
            _contentsChanged = true;
            _fileAlreadyExists = _fileSystem.Exists(_readName); // if we saved to or read from a file before
    ...
    internal static class ZipOutput
    {
                                                          ...
                                                          ZipContainer container)
        {
            ...
            if (needZip64CentralDirectory)
            {
                ...
                if (startSegment != 0)
                {
                    ...
                    int i = 16;
                    // number of this disk
                    ArrayExtensions.MemCopy(BitConverter.GetBytes(thisSegment), 0, a, i, 4);
                    i += 4;
                    // number of the disk with the start of the central directory
                    //ArrayExtensions.MemCopy(BitConverter.GetBytes(startSegment), 0, a, i, 4);
                    ArrayExtensions.MemCopy(BitConverter.GetBytes(thisSegment), 0, a, i, 4);
            ...
            if (startSegment != 0)
            {
                ...
                int i = 4;
                // number of this disk
                ArrayExtensions.MemCopy(BitConverter.GetBytes(thisSegment), 0, a2, i, 2);
                i += 2;
                // number of the disk with the start of the central directory
                //ArrayExtensions.MemCopy(BitConverter.GetBytes((UInt16)startSegment), 0, a2, i, 2);
                ArrayExtensions.MemCopy(BitConverter.GetBytes(thisSegment), 0, a2, i, 2);
                i += 2;
            }
                                                        ...
                                                        ZipContainer container)
        {
            ...
            ArrayExtensions.MemCopy(sig, 0, bytes, i, 4);
            i+=4;
                                                            ...
                                                            uint numSegments)
        {
            ...
            long DataSize = 44;
            ArrayExtensions.MemCopy(BitConverter.GetBytes(DataSize), 0, bytes, i, 8);
            i += 8;
            ...
            ArrayExtensions.MemCopy(BitConverter.GetBytes(numberOfEntries), 0, bytes, i, 8);
            i += 8;
            ArrayExtensions.MemCopy(BitConverter.GetBytes(numberOfEntries), 0, bytes, i, 8);
            i += 8;
            ...
            // offset 40
            Int64 SizeofCentraldirectory = EndOfCentralDirectory - StartOfCentralDirectory;
            ArrayExtensions.MemCopy(BitConverter.GetBytes(SizeofCentraldirectory), 0, bytes, i, 8);
            i += 8;
            ArrayExtensions.MemCopy(BitConverter.GetBytes(StartOfCentralDirectory), 0, bytes, i, 8);
            i += 8;
            ...
            ArrayExtensions.MemCopy(BitConverter.GetBytes(x2), 0, bytes, i, 4);
            i+=4;

            // offset 64
            // relative offset of the zip64 eocd
            ArrayExtensions.MemCopy(BitConverter.GetBytes(EndOfCentralDirectory), 0, bytes, i, 8);
            i += 8;

            // offset 72
            // total number of disks
            // (this will change later)
            ArrayExtensions.MemCopy(BitConverter.GetBytes(numSegments), 0, bytes, i, 4);
            i+=4;
