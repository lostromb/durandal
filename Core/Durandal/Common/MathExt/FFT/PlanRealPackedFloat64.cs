...
namespace Durandal.Common.MathExt.FFT
{
    internal class PlanRealPackedFloat64 : IReal1DFFTPlanFloat64
    {
        ...
        internal int length;
        internal int nfct;
        internal PooledBuffer<double> mem;
        ...
        public PlanRealPackedFloat64(int length)
        {
            ...
            {
                this.fct = new rfftp_fctdata_double[FFTConstants.NFCT];
            }
#endif
            ...
            this.mem = BufferPool<double>.Rent(tws);
            rfftp_comp_twiddle();
            DebugMemoryLeakTracer.TraceDisposableItemCreated(this);
        }

#if TRACK_IDISPOSABLE_LEAKS
        ~PlanRealPackedFloat64()
        {
            Dispose(false);
        }
#endif
        ...
        private static void rfftp_forward(PlanRealPackedFloat64 plan, Span<double> c, double fct)
        {
            ...
            PooledBuffer<double> scratch = null;
            bool doStackAlloc = n <= FFTConstants.STACKALLOC_DOUBLE_LIMIT;
            if (!doStackAlloc)
            {
                scratch = BufferPool<double>.Rent(n);
            }

            Span<double> ch = doStackAlloc ? stackalloc double[n] : scratch.Buffer;
            Span<double> p1 = c;
            Span<double> p2 = ch;
            bool swapped = false;
        ...
        private void rfftp_comp_twiddle()
        {
            ...
            for (int k = 0; k < this.nfct; ++k)
            {
                ...
                {
                    this.fct[k].tw = ptr;
                    Span<double> tw = this.mem.Buffer.AsSpan(ptr);
                    ptr += (ip - 1) * (ido - 1);
                    for (int j = 1; j < ip; ++j)
                    {
                ...
                {
                    this.fct[k].tws = ptr;
                    Span<double> tws = this.mem.Buffer.AsSpan(ptr);
                    ptr += 2 * ip;
                    tws[0] = 1.0;
                    tws[1] = 0.0;
