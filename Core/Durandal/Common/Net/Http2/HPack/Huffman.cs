...
namespace Durandal.Common.Net.Http2.HPack
{
    public class Huffman
    {
        ...
        private static void ResizeBuffer(
            ref PooledBuffer<byte> outBuf, int usedBytes, int newBytes)
        {
            var newSize = outBuf.Length + newBytes;
            var newBuf = BufferPool<byte>.Rent(newSize);
            ArrayExtensions.MemCopy(outBuf.Buffer, 0, newBuf.Buffer, 0, usedBytes);
            outBuf.Dispose();
            outBuf = newBuf;
        }
        ...
        public static string Decode(ArraySegment<byte> input)
        {
            ...
            var outBuf = BufferPool<byte>.Rent(estLength);
                ...
                for (inputBitOffset = 7; inputBitOffset >= 0; inputBitOffset--) {
                    ...
                    if (treeNode.Value != -1)
                    {
                        if (treeNode.Value != 256)
                        {
                            ...
                            outBuf.Buffer[byteCount] = (byte)treeNode.Value;
                            byteCount++;
            ...
            var str = Encoding.UTF8.GetString(outBuf.Buffer, 0, byteCount);
            outBuf.Dispose();
            return str;
        }
