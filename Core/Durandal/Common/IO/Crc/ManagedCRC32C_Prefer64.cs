...
{
    /// <summary>
    /// Basically the same as <see cref="ManagedCRC32C"/> except that it favors 64-bit hardware
    /// and performs slightly better there.
    /// </summary>
    internal class ManagedCRC32C_Prefer64 : ICRC32C
    {
        /// <inheritdoc />
        ...
        public void Slurp(ref CRC32CState state, ReadOnlySpan<byte> input_8)
        {
...
#endif
            ulong crc = state.Checksum ^ 0xffffffff;

            // align the input byte span to an 8-byte boundary, both
            ...
            {
                crc ^= input_64[input_idx_64];
                ulong high = input_64[input_idx_64 + 1];
                crc = CrcTables.CrcTable32[15][crc & 0xff]
                    ^ CrcTables.CrcTable32[14][(crc >> 8) & 0xff]
                    ^ CrcTables.CrcTable32[13][(crc >> 16) & 0xff]
                    ^ CrcTables.CrcTable32[12][(crc >> 24) & 0xff]
                    ^ CrcTables.CrcTable32[11][(crc >> 32) & 0xff]
                    ^ CrcTables.CrcTable32[10][(crc >> 40) & 0xff]
                    ^ CrcTables.CrcTable32[9][(crc >> 48) & 0xff]
                    ^ CrcTables.CrcTable32[8][crc >> 56]
                    ^ CrcTables.CrcTable32[7][high & 0xff]
                    ^ CrcTables.CrcTable32[6][(high >> 8) & 0xff]
                    ^ CrcTables.CrcTable32[5][(high >> 16) & 0xff]
                    ^ CrcTables.CrcTable32[4][(high >> 24) & 0xff]
                    ^ CrcTables.CrcTable32[3][(high >> 32) & 0xff]
                    ^ CrcTables.CrcTable32[2][(high >> 40) & 0xff]
                    ^ CrcTables.CrcTable32[1][(high >> 48) & 0xff]
                    ^ CrcTables.CrcTable32[0][high >> 56];
                input_idx_8 += 2 * sizeof(ulong);
                input_idx_64 += 2;
            ...
            state.Checksum = (uint)(crc ^ 0xffffffff);
        }
    }
}
