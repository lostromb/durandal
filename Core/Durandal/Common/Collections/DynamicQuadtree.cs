...
namespace Durandal.Common.Collections
{
    ...
    public class DynamicQuadtree<T>
    {
        ...
        /// <returns></returns>
        public IEnumerable<Tuple<T, Vector2f>> GetItemsInRect(Rect2f boundaries)
        {
        ...
        public IEnumerable<Tuple<T, Vector2f>> GetItemsNearPoint(Vector2f point)
        {
            if (_root.Weight == 0)
            {
                yield break;
            ...
            while (itemsFound == 0)
            {
                foreach (var x in GetItemsInRect(new Rect2f(
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<Tuple<T, Vector2f>> GetItemsNearPoint(Vector2f point, float maxDistance)
        {
            point = PutVectorInBounds(point);
            QuadtreeNode<T> bottomNode = GetContainingNode(point);
            int itemsReturned = 0;
            
            while (itemsReturned == 0 && bottomNode.Parent != null)
            {
                bottomNode = bottomNode.Parent;
                foreach (var nearbyItem in bottomNode.GetItemsNear(point, maxDistance))
                {
                    yield return nearbyItem;
                    itemsReturned++;
                }
            }
        }
        ...
        private class QuadtreeNode<E>
        {
            ...
            public IEnumerable<Tuple<E, Vector2f>> GetItems()
            {
                ...
                else
                {
                    for (int c = 0; c < 4; c++)
                    {
                        foreach (var item in _children[c].GetItems())
                        {
                            yield return item;
                        }
                    }
            ...
            public IEnumerable<Tuple<E, Vector2f>> GetItemsNear(Vector2f point, float maxDist)
            {
                if (!_isSubdivided)
                {
                ...
                else
                {
                    for (int c = 0; c < 4; c++)
                    {
                        foreach (var item in _children[c].GetItems())
                        {
                            if (point.Distance(item.Item2) < maxDist)
                            {
                                yield return item;
                            }
                        }
