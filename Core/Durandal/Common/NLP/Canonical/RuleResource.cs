...
namespace Durandal.Common.NLP.Canonical
{
    ...
    /// </summary>
    public class RuleResource : Resource
    {
        #region Fields
        ...
        private string _markedPartNumber;
        private string _normalizerName;
        private readonly IList<string> _includeRestrictions;

        private readonly IDictionary<string, RegexResource> _regexResources;
        private readonly IDictionary<string, NormalizationResource> _normalizationResources;

        #endregion

        public RuleResource(IDictionary<string, RegexResource> regexResources,
                            IDictionary<string, NormalizationResource> normalizationResources,
        ...
        private static XElement ExtractAndRemoveNormalizerElement(XElement element)
        {
            var tagElement = element.Element(GrammarElements.Normalizer);
            if (tagElement != null)
                tagElement.Remove();
        ...
        private void ParseRuleref(XElement ruleref, StringBuilder expressionBuilder)
        {
            ...
            if (uriAttribute == null)
                throw new FormatException(string.Format("Incorrect format. Rule {0}. Ruleref uri is not specified", Id));
            ...
            if (!_regexResources.ContainsKey(ruleId))
                throw new FormatException(string.Format("Incorrect format. Rule {0}. Ruleref uri is not valid", Id));
        ...
        private void ParseItem(XElement item, StringBuilder expressionBuilder)
        {
            ...
            var normalizerTag = ExtractAndRemoveNormalizerElement(item);

            // generate regular expression
            if (normalizerTag != null || include != null || !string.IsNullOrEmpty(repeat))
            {
                var partName = string.Format(RegexPartTemplate, _markedPartsCount);

                expressionBuilder.AppendFormat("(?<{0}>", partName);

                if (normalizerTag != null)
                {
                    _markedPartNumber = partName;
                    _normalizerName = normalizerTag.Value;
                    if (!_normalizationResources.ContainsKey(_normalizerName))
                        throw new FormatException("Normalization rule " + _normalizerName + " referenced in rule " + Id + " does not exist");
                }
        ...
        private void ParseOneOf(XElement oneOf, StringBuilder expressionBuilder)
        {
            ...
            var normalizerTag = ExtractAndRemoveNormalizerElement(oneOf);
        ...
        public override void Parse(XElement resource)
        {
            ...
            if (exampleAttribute != null &&
                (!_compiledExactExpression.IsMatch(exampleAttribute.Value) ||
                !_compiledExpression.IsMatch(exampleAttribute.Value)))
                throw new FormatException(string.Format("Incorrect format. Rule {0}. Rule does not match specified example", Id));
        }
        ...
        public RuleMatch RuleMatch(string text, ILogger queryLogger)
        {
            if (text == null)
                return null;
            ...
            if (match.Success)
            {
                ruleMatch = ToRuleMatch(match, queryLogger);
            }
        ...
        /// <param name="queryLogger">A logger for errors</param>
        /// <returns>RuleMatch object that contains matched expression and extracted values</returns>
        public IList<RuleMatch> RuleMatches(string text, ILogger queryLogger)
        {
            if (text == null)
                return null;
            ...
            {
                var ruleMatch = ToRuleMatch(match, queryLogger);
        ...
        private RuleMatch ToRuleMatch(Match match, ILogger queryLogger)
        {
            if (match == null)
                return null;

            string nonNormalizedValue = string.Empty;
            string normalizedTagValue = string.Empty;

            // Only return a normalized value if one part of the regex was "marked" with a <normalizer> tag.
            // For negative_rules, this should always be absent.
            if (_markedPartNumber != null)
            {
                nonNormalizedValue = match.Groups[_markedPartNumber].Value;
                normalizedTagValue = _normalizationResources[_normalizerName].Normalize(nonNormalizedValue, queryLogger);
            }
            ...
            var ruleMatch = new RuleMatch
                {
                    ...
                    Index = match.Index,
                    Rule = this,
                    NormalizedTagValue = normalizedTagValue
                };
