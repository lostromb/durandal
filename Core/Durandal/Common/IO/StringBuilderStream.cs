...
namespace Durandal.Common.IO
{
    ...
    /// </summary>
    public class StringBuilderStream : NonRealTimeStream
    {
        private const int BASE_BUFFER_SIZE = 1024;
        ...
        public StringBuilderStream(StringBuilder input, Encoding encoding, bool maskNewline = false)
        ...
        /// <param name="encoding">The encoding to use</param>
        /// <param name="maskNewline">If true, all newline \r \n characters in the input string buffer will be replaced with spaces on copy.</param>
        public StringBuilderStream(StringBuilder input, int charOffset, int charCount, Encoding encoding, bool maskNewline = false)
        {
            ...
            _byteScratch = BufferPool<byte>.Rent(BASE_BUFFER_SIZE * 4); // we assume for any given encoding, 1 char will generate a max of 4 bytes
