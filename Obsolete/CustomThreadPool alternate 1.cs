...
namespace Durandal.Common.Tasks
{
    ...
    public class CustomThreadPool : IThreadPool
    {
        ...
        private class WorkerThread : TaskScheduler
        {
                ...
                bool hideExceptions)
            {
                ...
                _taskFactory = new TaskFactory(_threadAbort, TaskCreationOptions.DenyChildAttach, TaskContinuationOptions.DenyChildAttach, this);
                ...
                _threadLogger.Log("Managed thread ID is " + _systemThread.ManagedThreadId, LogLevel.Vrb);
            }
            ...
            {
                //_threadLogger.Log("Queueing an internal task", LogLevel.Vrb);
                _workItems.Enqueue(new InternalWorkItem(task));
                _queueActivity.Set();
            }
            ...
            /// </summary>
            /// <param name="task">The work item to run</param>
            /// <param name="taskWasPreviouslyQueued">Whether this work item was previously sent via QueueTask()</param>
            /// <returns></returns>
            protected sealed override bool TryExecuteTaskInline(Task task, bool taskWasPreviouslyQueued)
            {
                // Are we executing on the actual worker thread?
                if (_systemThread.ManagedThreadId != Thread.CurrentThread.ManagedThreadId)
                {
                    return false;
                }

                //_threadLogger.Log("Attempting to run task inline on thread " + Thread.CurrentThread.ManagedThreadId, LogLevel.Vrb);
                ...
                if (taskWasPreviouslyQueued)
                {
                    ...
                    if (TryDequeue(task))
                    {
                        //_threadLogger.Log("Running task inline on thread " + Thread.CurrentThread.ManagedThreadId, LogLevel.Vrb);
                        return base.TryExecuteTask(task);
                    }
                    else
                    {
                        //_threadLogger.Log("Not running task inline", LogLevel.Vrb);
                        return false;
                    }
                }
                else
            ...
            {
                return _workItems.Remove(new InternalWorkItem(task));
            }

            /// <summary>
            /// Gets an enumerable of the tasks currently scheduled on this scheduler.
            /// </summary>
            /// <returns>The currently scheduled task list.</returns>
            protected sealed override IEnumerable<Task> GetScheduledTasks()
            {
                // _workItems is concurrently enumerable so this is fine
                return _workItems
                    .Where((t) => t.AsyncTask != null)
                    .Select((t) => t.AsyncTask);
            }
                        ...
                        if (!_threadAbort.IsCancellationRequested)
                        {
                            try
                            {
                                ...
                                else if (nextAction.TaskProducer != null)
                                {
                                    ...
                                    //Task runningWhat = nextAction.TaskProducer();
                                    //QueueTask(runningWhat);
