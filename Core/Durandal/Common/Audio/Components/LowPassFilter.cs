...
namespace Durandal.Common.Audio.Components
{
    public sealed class LowPassFilter : BiquadFilter
    {
        ...
        /// <summary>
        /// Constructs a new lowpass filter.
        /// </summary>
        ...
        public LowPassFilter(WeakPointer<IAudioGraph> graph, AudioSampleFormat format, string nodeCustomName, float frequencyHz, float q = 0.70721f)
            : base(graph, format, nameof(LowPassFilter), nodeCustomName)
        {
            if (frequencyHz < 0)
            {
