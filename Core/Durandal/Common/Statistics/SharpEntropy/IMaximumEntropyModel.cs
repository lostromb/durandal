...
namespace Durandal.Common.Statistics.SharpEntropy
{
	...
	public interface IMaximumEntropyModel
	{
		...
		/// </returns>
        float[] Evaluate(string[] context, float[] probabilities);
		...
		/// </returns>
        string GetBestOutcome(float[] outcomes);
		...
		/// for each one.
		/// </returns>
        string GetAllOutcomes(float[] outcomes);
