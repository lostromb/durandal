namespace Durandal.Common.NLP.Tagging
{
    ...
    public class RecipeCrfTagger
    {
        ...
        /// <param name="resourceManager">A resource manager for accessing training and cache files</param>
        /// <param name="wordBreaker">Wordbreaker</param>
        public RecipeCrfTagger(IStatisticalTrainer modelTrainer, ILogger logger, float minConfidence, IFileSystem resourceManager, IWordBreaker wordBreaker)
        {
            _logger = logger;
            _stringIndex = BasicCompactIndex<string>.BuildStringIndex();
            _nodeNameMapping = new Dictionary<Compact<string>, int>();
            _confidenceCutoff = minConfidence;
            _resourceManager = resourceManager;
            _wordBreaker = wordBreaker;
            _modelTrainer = new MaxEntClassifierTrainer(logger, resourceManager);
        }
        ...
        /// <param name="domainAndIntent"></param>
        /// <param name="cacheFileName"></param>
        public void TrainFromData(
            TrainingDataList<TagFeature> tagData,
            VirtualPath cacheFileRoot)
        {
            ...
            Compact<string> nodeName = _stringIndex.Store(STKN);
            _nodeNameMapping[nodeName] = 0;
            nodeNames.Add(STKN);
            trainingData.Add(new TrainingDataList<SimpleOutcomeFeature>(SimpleOutcomeFeature.CreateStatic));

            //if (!IsTrainingRequired(_modelTrainer, cacheFileRoot, _resourceManager, domainConfig, _logger))
            //{
            //    // Load the cache
            //    TrainingDataList<SimpleOutcomeFeature> dummy = new TrainingDataList<SimpleOutcomeFeature>(SimpleOutcomeFeature.CreateStatic);
            //    foreach (string n in domainConfig.GetVectorString("nodenames_" + intent))
            //    {
            //        Compact<string> one = _stringIndex.Store(n);
            ...
            //    {
            //        string thisClassifierTag = _stringIndex.Retrieve(classifierTag);
            //        int nodeIndex = _nodeNameMapping[classifierTag];
            //        classifiers[nodeIndex] = _modelTrainer.TrainClassifier(new FeatureReader(dummy), cacheFileRoot + " " + thisClassifierTag, _stringIndex, thisClassifierTag, MODEL_QUALITY);
            //    }
            //}
            //else
            {
                ...
                {
                    Compact<string> one = _stringIndex.Store(feature.SourceTag);
                    if (!_nodeNameMapping.ContainsKey(one))
                    {
                        _nodeNameMapping[one] = _nodeNameMapping.Count;
                ...
                foreach (Compact<string> classifierTag in _nodeNameMapping.Keys)
                {
                    string thisClassifierTag = _stringIndex.Retrieve(classifierTag);
                    int nodeIndex = _nodeNameMapping[classifierTag];
                    classifiers[nodeIndex] = _modelTrainer.TrainClassifier(new FeatureReader(trainingData[nodeIndex]), cacheFileRoot + " " + thisClassifierTag, _stringIndex, thisClassifierTag, MODEL_QUALITY);
                }
            }
        }
        ...
        private HypothesisNode[] GenerateTaggerLattice(Sentence utterance, ITagFeatureExtractor featureExtractor)
        {
            ...
            rootNode.ThisClassifierNode = _stringIndex.GetIndex(STKN);
        ...
        private void ClassifyOne(HypothesisNode currentNode, Sentence utterance, ITagFeatureExtractor extractor, ref List<HypothesisNode> output)
        {
            ...
            if (currentNode.ThisClassifierNode == _stringIndex.GetIndex(ETKN) ||
                currentNode.ThisClassifierNode == _stringIndex.GetNullIndex())
            {
                ...
                newHyp.ThisClassifierNode = _stringIndex.GetIndex(ETKN);
            ...
            else
            {
                ...
                foreach (Hypothesis<string> posResult in posResults)
                {
                    ...
                    newHyp.ThisClassifierNode = _stringIndex.GetIndex(posResult.Value);
                    newHyp.Backpointer = currentNode;
