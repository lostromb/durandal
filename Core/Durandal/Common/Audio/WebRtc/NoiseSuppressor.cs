...
namespace Durandal.Common.Audio.WebRtc
{
	public class NoiseSuppressor
	{
		...
		private void WebRtcNs_NoiseEstimation(float[] magn, float[] noise)
		{
            ...
            ArrayExtensions.MemCopy(quantile, 0, noise, 0, magnLen);
		}
		...
		private void WebRtcNs_FeatureParameterExtraction(int flag)
		{
			...
			if (flag == 1)
			{
				...
				if (modelUpdatePars[0] >= 1)
				{
                    ...
                    ArrayExtensions.WriteZeroes(histLrt, 0, HIST_PAR_EST);
                    ArrayExtensions.WriteZeroes(histSpecFlat, 0, HIST_PAR_EST);
                    ArrayExtensions.WriteZeroes(histSpecDiff, 0, HIST_PAR_EST);
		...
		public void ProcessFrame(short[] inFrame, int inOffset, short[] outFrame, int outOffset)
		{
            ...
            ArrayExtensions.MemCopy(dataBuf, blockLen10Ms, dataBuf, 0, (anaLen - blockLen10Ms));
            ArrayExtensions.MemCopy(fin, 0, dataBuf, (anaLen - blockLen10Ms), blockLen10Ms);
			...
			if (outLen == 0)
			{
                ...
                ArrayExtensions.MemCopy(syntBuf, blockLen, syntBuf, 0, (anaLen - blockLen));
				ArrayExtensions.WriteZeroes(syntBuf, anaLen - blockLen, blockLen);

				// out buffer
				outLen = blockLen - blockLen10Ms;
				if (blockLen > blockLen10Ms)
				{
                    ArrayExtensions.MemCopy(fout, blockLen10Ms, outBuf, 0, outLen);
				}
			} // end of if out.len==0
			else
			{
                ArrayExtensions.MemCopy(fout, 0, outBuf, 0, outLen);
                ArrayExtensions.MemCopy(outBuf, blockLen10Ms, outBuf, 0, (outLen - blockLen10Ms));
                ArrayExtensions.WriteZeroes(outBuf, outLen - blockLen10Ms, blockLen10Ms);
				outLen -= blockLen10Ms;
			}
