...
namespace Durandal.Common.Compression.Zip
{
    ...
    public class ZipInputStream : Stream
    {
        ...
        /// </code>
        /// </example>
        public ZipInputStream(Stream stream, ILogger logger, IFileSystem fileSystem)  : this (stream, logger, fileSystem, false) { }
        ...
        /// </code>
        /// </example>
        public ZipInputStream(ILogger logger, VirtualPath fileName, IFileSystem fileSystem)
        {
            Stream stream = fileSystem.OpenStream(fileName, FileOpenMode.Open, FileAccessMode.Read);
            _Init(stream, logger, fileSystem, false, fileName);
        }
        ...
        /// </param>
        public ZipInputStream(Stream stream, ILogger logger, IFileSystem fileSystem, bool leaveOpen)
        {
            _Init(stream, logger, fileSystem, leaveOpen, null);
        }

        private void _Init(Stream stream, ILogger logger, IFileSystem fileSystem, bool leaveOpen, VirtualPath name)
        {
            _inputStream = stream;
            _logger = logger;
            ...
            _provisionalAlternateEncoding = SharedUtilities.GetIBM437Encoding();
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (_LeftToRead == 0)
            {
                ...
                // workitem 10178
                Durandal.Common.Compression.Zip.SharedUtilities.Workaround_Ladybug318918(_inputStream);
            }
        ...
        public ZipEntry GetNextEntry()
        {
            ...
            _currentEntry = ZipEntry.ReadEntry(_container, _logger, _fileSystem, !_firstEntry);
...
#pragma warning disable CA2213 // I don't know enough about this code to try and change the disposal pattern
        private Durandal.Common.Compression.CrcCalculatorStream _crcStream;
