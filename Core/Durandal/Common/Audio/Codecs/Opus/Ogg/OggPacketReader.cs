...
#pragma warning disable 67  // disable the "unused" warning
        public AsyncEvent<ParameterChangeEventArgs> ParameterChangeEvent { get; private set; }
#pragma warning restore 67
        ...
        internal PacketReader(OggContainerReader container, int streamSerial)
        {
            ...
            ParameterChangeEvent = new AsyncEvent<ParameterChangeEventArgs>();
        }
        ...
        public async Task<DataPacket> GetNextPacket()
        {
            return (_current = await PeekNextPacketInternal().ConfigureAwait(false));
        }

        public async Task<DataPacket> PeekNextPacket()
        {
            return await PeekNextPacketInternal().ConfigureAwait(false);
        }

        private async Task<Packet> PeekNextPacketInternal()
        {
            ...
            else
            {
                while (true)
                {
                    ...
                    await _container.GatherNextPage(_streamSerial).ConfigureAwait(false);
                }
            }
        ...
        internal async Task ReadAllPages()
        {
        ...
        internal async Task<DataPacket> GetLastPacket()
        {
            await ReadAllPages().ConfigureAwait(false);
        ...
        public async Task<DataPacket> GetPacket(int packetIndex)
        {
            if (!CanSeek) throw new InvalidOperationException();
        ...
        public async Task<DataPacket> FindPacket(long granulePos, Func<DataPacket, DataPacket, int> packetGranuleCountCallback)
        {
        ...
        public async Task<long> GetGranuleCount()
        {
            return (await GetLastPacket().ConfigureAwait(false)).PageGranulePosition;
        }
    }
}
