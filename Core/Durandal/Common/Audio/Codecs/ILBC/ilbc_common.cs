 ...
 * @author Jean Lorchat
 */
static class ilbc_common {

   /*----------------------------------------------------------------*
    *  check for stability of lsf coefficients
    *---------------------------------------------------------------*/

    internal static int LSF_check(    /* (o) 1 for stable lsf vectors and 0 for
                              nonstable ones */
    ...
    *---------------------------------------------------------------*/
    ...
    internal static void StateConstructW(
       int idxForMax,      /* (i) 6-bit index for the quantization of
                                  max amplitude */
       int[] idxVec,    /* (i) vector of quantization indexes */
       float[] syntDenum,   /* (i) synthesis filter denumerator */
       int syntDenum_idx,
       float[] _out,         /* (o) the decoded state vector */
       int out_idx,
       ...
       maxVal = (float)System.Math.Pow(10,maxVal) / 4.5f;
     ...
     //System.out.println("idxVec["+li+"] = " + idxVec[li]);
       ...
       ilbc_common.ZeroPoleFilter(tmpbuf, tmp, numerator, syntDenum, syntDenum_idx,
                  ...
                  foutbuf, fout);
       for (k=0;k<len;k++) {
           _out[out_idx+k] = foutbuf[fout+len-1-k]+foutbuf[fout+2*len-1-k];
    ...
    *---------------------------------------------------------------*/
    ...
    internal static void AllPoleFilter(
    ...
    *---------------------------------------------------------------*/
    ...
    internal static void AllZeroFilter(
    ...
    *---------------------------------------------------------------*/
    ...
    internal static void ZeroPoleFilter(
       float[] In,      /* (i) In[0] to In[lengthInOut-1] contain
    ...
    *---------------------------------------------------------------*/
    ...
    internal static void lsf2a(float[] a_coef, float[] freq)
    {
    int i, j;
    float hlp;
...
//        memset(b, 0, (LPC_HALFORDER+1)*sizeof(float));
       ...
       for (i = 0; i < ilbc_constants.LPC_HALFORDER; i++) {
           p[i] = (float)System.Math.Cos(ilbc_constants.TWO_PI * freq[2 * i]);
           q[i] = (float)System.Math.Cos(ilbc_constants.TWO_PI * freq[2 * i + 1]);
       }
    ...
    *---------------------------------------------------------------*/
    ...
    internal static void interpolate(
       float[] _out,      /* (o) the interpolated vector */
       ...
       int length)      /* (i) length of all vectors */
   {
       ...
       invcoef = 1.0f - coef;
       for (i = 0; i < length; i++) {
           _out[i] = coef * in1[i] + invcoef * in2[i + in2_idx];
    ...
    *---------------------------------------------------------------*/
    ...
    internal static void bwexpand(
         float[] _out,      /* (o) the bandwidth expanded lpc
                              coefficients */
         int out_idx,
         float[] _in,      /* (i) the lpc coefficients before bandwidth
                              expansion */
    ...
    _out[out_idx] = _in[0];
    for (i = 1; i < length; i++) {
        _out[i + out_idx] = chirp * _in[i];
        chirp *= coef;
    }
    }

    internal static void getCBvec(
    ...
    float alfa, alfa1;
       ...
       if (index<lMem-cbveclen+1) {
           ...
           k=index+cbveclen;
           /* get vector */
       System.Array.Copy(mem, mem_idx + lMem - k, cbvec, 0, cbveclen);
       ...
       System.Array.Copy(mem, mem_idx + lMem - k / 2, cbvec, 0, ilow);
       ...
       System.Array.Copy(mem, mem_idx+lMem-k+ihigh, cbvec, ihigh, (cbveclen-ihigh));
...
//                    CB_HALFFILTERLEN*sizeof(float));
           System.Array.Copy(mem, mem_idx, tempbuff2, ilbc_constants.CB_HALFFILTERLEN, lMem);
...
//                    pp1=&cbfiltersTbl[CB_FILTERLEN-1];
           ...
           System.Array.Copy(tmpbuf, lMem - k / 2, cbvec, 0, ilow);
...
//                    ilow*sizeof(float));
           ...
           System.Array.Copy(tmpbuf, lMem-k+ihigh, cbvec, ihigh, cbveclen - ihigh);
...
//                    (cbveclen-ihigh)*sizeof(float));
    ...
    internal static float gainquant(/* (o) quantized gain value */
            float _in,       /* (i) gain value */
    ...
    {
    int i, tindex;
    float minmeasure,measure, scale;
    float[] cb;
    ...
    tindex=0;
    for (i=0; i<cblen; i++) {
        measure = (_in - scale*cb[i])*(_in-scale*cb[i]);
    ...
    internal static float gaindequant(  /* (o) quantized gain value */
    ...
    internal static void iCBConstruct(
       ...
       if (nStages > 1) {
           gain[1] = gaindequant(gain_index[gain_index_idx + 1],
               System.Math.Abs(gain[0]), 16);
       }
       if (nStages > 2) {
           gain[2] = gaindequant(gain_index[gain_index_idx + 2],
               System.Math.Abs(gain[1]), 8);
       }
