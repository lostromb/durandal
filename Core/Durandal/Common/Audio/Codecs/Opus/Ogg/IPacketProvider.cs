...
namespace Durandal.Common.Audio.Codecs.Opus.Ogg
{
    ...
    internal interface IPacketProvider : IDisposable
    {
        ...
        /// <exception cref="InvalidOperationException"><see cref="CanSeek"/> is <c>False</c>.</exception>
        Task<DataPacket> GetPacket(int packetIndex);
        ...
        Task<DataPacket> FindPacket(long granulePos, Func<DataPacket, DataPacket, int> packetGranuleCountCallback);
        ...
        /// </summary>
        AsyncEvent<ParameterChangeEventArgs> ParameterChangeEvent { get; }
    }
}
