...
namespace Durandal.Common.Compression.ZLib
{
    ...
    ///   <c>Durandal.Common.Compression.ZLib.GZipStream</c> can compress while writing, or decompress while
    ...
    public class GZipStream : System.IO.Stream
    {
        ...
        ///     {
        ///         using (Stream decompressor= new Durandal.Common.Compression.ZLib.GZipStream(input, CompressionMode.Decompress, true))
        ///         {
        ...
        ///         Using decompressor As Stream = new Durandal.Common.Compression.ZLib.GZipStream(input, CompressionMode.Decompress, True)
        ...
        public override long Position
        {
            get
            {
                if (this._baseStream._streamMode == Durandal.Common.Compression.ZLib.ZlibBaseStream.StreamMode.Writer)
                    return this._baseStream._z.TotalBytesOut + _headerByteCount;
                if (this._baseStream._streamMode == Durandal.Common.Compression.ZLib.ZlibBaseStream.StreamMode.Reader)
        ...
        {
            if (_disposed) throw new ObjectDisposedException(nameof(GZipStream));
            int n = await _baseStream.ReadAsync(buffer, offset, count, cancelToken);
        ...
        {
            if (_disposed) throw new ObjectDisposedException(nameof(GZipStream));
            if (_baseStream._streamMode == Durandal.Common.Compression.ZLib.ZlibBaseStream.StreamMode.Undefined)
            {
        ...
        private int EmitHeader()
        {
            ...
            ArrayExtensions.MemCopy(BitConverter.GetBytes(timet), 0, header, i, 4);
            i += 4;
            ...
            if (fnLength != 0)
            {
                ArrayExtensions.MemCopy(filenameBytes, 0, header, i, fnLength - 1);
                i += fnLength - 1;
            ...
            if (cbLength != 0)
            {
                ArrayExtensions.MemCopy(commentBytes, 0, header, i, cbLength - 1);
                i += cbLength - 1;
                header[i++] = 0; // terminate
            }
        ...
        public static byte[] UncompressBuffer(byte[] compressed)
        {
            ...
            using (System.IO.Stream decompressor = new GZipStream(input, CompressionMode.Decompress))
            {
