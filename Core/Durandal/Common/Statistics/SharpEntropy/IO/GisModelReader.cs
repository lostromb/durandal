...
namespace Durandal.Common.Statistics.SharpEntropy.IO
{
	...
	public abstract class GisModelReader : IGisModelReader
	{
        ...
        public GisModelReader(ICompactIndex<string> stringIndex)
        {
		...
		/// </summary>
		/// <remarks>
		/// This method assumes that models are saved in the
		/// following sequence:
		/// 
		...
		protected virtual void CheckModelType()
		{
			...
			if (modelType != "GIS") 
			{
				throw new FormatException("Error: attempting to load a " + modelType + " model as a GIS model." + " You should expect problems.");
			}
		}
		...
		/// </summary>
        protected virtual float ReadCorrectionParameter()
		{
			return ReadFloat();
		}
		...
		/// </summary>
        protected virtual void ReadPredicates(out int[][] outcomePatterns, out Dictionary<Compact<string>, PatternedPredicate> predicates)
		{
		...
		/// </summary>
        protected virtual Dictionary<Compact<string>, PatternedPredicate> ReadParameters(int[][] outcomePatterns, string[] predicateLabels)
		{
            Dictionary<Compact<string>, PatternedPredicate> predicates = new Dictionary<Compact<string>, PatternedPredicate>(predicateLabels.Length);
			...
			for (int currentOutcomePattern = 0; currentOutcomePattern < outcomePatterns.Length; currentOutcomePattern++)
			{
				...
				{
                    float[] parameters = new float[outcomePatterns[currentOutcomePattern].Length - 1];
					...
					predicates.Add(_stringIndex.Store(predicateLabels[parameterIndex]), new PatternedPredicate(currentOutcomePattern, parameters));
					parameterIndex++;
				}
			}
			return predicates;
		...
		/// predicate.
		/// </param>
        public virtual void GetPredicateData(string predicateLabel, int[] featureCounts, float[] outcomeSums)
		{
		    Compact<string> test = _stringIndex.GetIndex(predicateLabel);
            if (mPredicates.ContainsKey(test))
            {
                PatternedPredicate predicate = mPredicates[test];
