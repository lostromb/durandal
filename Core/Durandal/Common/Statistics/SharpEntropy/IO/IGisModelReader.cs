...
namespace Durandal.Common.Statistics.SharpEntropy.IO
{
	...
	public interface IGisModelReader
	{
		...
		/// usually only be accessed by GIS model writer classes via the GisModel class.
		/// </summary>
        Dictionary<Compact<string>, PatternedPredicate> GetPredicates();
		...
		/// predicate.
		/// </param>
        void GetPredicateData(string predicateLabel, int[] featureCounts, float[] outcomeSums);
