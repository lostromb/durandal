...
{
    public class TcpClientSocketFactory : ISocketFactory
    {
        ...
        public async Task<ISocket> Connect(
            TcpConnectionConfiguration opts,
            ILogger traceLogger = null,
            CancellationToken cancelToken = default(CancellationToken),
            IRealTimeProvider realTime = null)
        {
            ...
            try
            {
                ...
                Task connectTask = socket.ConnectAsync(opts.DnsHostname, opts.Port.Value);
...
#else
                await socket.ConnectAsync(opts.DnsHostname, opts.Port.Value, linkedCts.Token).ConfigureAwait(false);
#endif
                cancelToken.ThrowIfCancellationRequested();
                ...
                {
                    TcpClientSocket returnVal = new TcpClientSocket(socket);
                    socket = null;
                    return returnVal;
                }
            }
