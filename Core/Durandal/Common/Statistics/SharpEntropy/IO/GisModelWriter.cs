...
namespace Durandal.Common.Statistics.SharpEntropy.IO
{
	...
	public abstract class GisModelWriter
	{
        ...
        public GisModelWriter(ICompactIndex<string> stringIndex)
        {
		...
		/// </summary>
		/// <param name="data">
		/// Single precision floating point data to be written to storage.
		/// </param>
		protected abstract void WriteFloat(float data);
		...
		protected virtual void Initialize(GisModel model)
		{
            ...
            Dictionary<Compact<string>, PatternedPredicate> predicates = model.GetPredicates();
			//build arrays of predicates and predicate names from the dictionary
			mPredicates = new PatternedPredicate[predicates.Count];
            Compact<string>[] predicateNames = new Compact<string>[predicates.Count];
        ...
        protected virtual void WriteCorrectionParameter(float correctionParameter)
		{
			WriteFloat(correctionParameter);
		}
		...
		protected void WritePredicateNames()
		{
			...
			{
				WriteString(_stringIndex.Retrieve(mPredicates[currentPredicate].Name));
			}
		}
		...
		protected void WriteParameters()
		{
			for (int currentPredicate = 0; currentPredicate < mPredicates.Length; currentPredicate++)
			{
				...
				{
					WriteFloat(mPredicates[currentPredicate].GetParameter(currentParameter));
				}
			}
		}
