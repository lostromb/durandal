...
namespace Durandal.Common.Audio.WebRtc
{
	...
	public class RingBuffer
	{
		...
		public void Write(short[] input, int length)
		{
			...
			if (rwWrap == Wrap.SameWrap)
			{
				...
				if (n > margin)
				{
                    ...
                    // ArrayExtensions.MemCopy(data, 0, this.data, writePos, margin);
                    ArrayExtensions.MemCopy(input, 0, data, writePos, margin);
				...
				{
                    // ArrayExtensions.MemCopy(data, 0, this.data, writePos, n);
                    ArrayExtensions.MemCopy(input, 0, data, writePos, n);
					writePos += n;
					return;
				}
			}
			...
			if (rwWrap == Wrap.DiffWrap)
			{
                ...
                ArrayExtensions.MemCopy(input, (length - n), data, writePos, margin);
				// ArrayExtensions.MemCopy(data, size - n, this.data, writePos, margin);
				writePos += margin;
			}
		}
		...
		public void Read(short[] output, int length)
		{
			...
			if (rwWrap == Wrap.DiffWrap)
			{
				...
				{
					rwWrap = Wrap.SameWrap;
                    ArrayExtensions.MemCopy(data, readPos, output, 0, margin);
					// ArrayExtensions.MemCopy(this.data, readPos, data, 0, margin);
				...
				{
                    ArrayExtensions.MemCopy(data, readPos, output, 0, n);
					// ArrayExtensions.MemCopy(this.data, readPos, data, 0, n);
			...
			if (rwWrap == Wrap.SameWrap)
			{
                ...
                ArrayExtensions.MemCopy(data, readPos, output, (length - n), margin);
				// ArrayExtensions.MemCopy(this.data, readPos, data, size - n, margin);
	...
	public class AecCore
	{
		...
		private readonly Fourier fft = new Fourier(AecConfig.PART_LEN1);
		...
		private readonly ComplexF[] sde = new ComplexF[AecConfig.PART_LEN1]; // cross-psd of nearend and error
		private readonly ComplexF[] sxd = new ComplexF[AecConfig.PART_LEN1]; // cross-psd of farend and nearend
		...
		private readonly ComplexF[] xfwBuf; // farend windowed fft buffer
...
*/
		...
		*/
		...
		public AecCore(AecConfig aecConfig)
		{
			...
			xfwBuf = new ComplexF[aecConfig.NumPartitions * AecConfig.PART_LEN1]; // farend windowed fft buffer
		...
		private void BufferFar(short[] farend, int farLen)
		{
			...
			while (farBufWritePos + writeLen > aecConfig.FarBufferLength)
			{
                ...
                ArrayExtensions.MemCopy(farend, writePos, farBuf, farBufWritePos, writeLen);
				// ArrayExtensions.MemCopy(farend, writePos, farBuf, farBufWritePos, writeLen);
				farBufWritePos = 0;
				writePos = writeLen;
		...
		private void FetchFar(short[] farend, int farLen, int knownDelay)
		{
			...
			while (farBufReadPos + readLen > aecConfig.FarBufferLength)
			{
                ...
                ArrayExtensions.MemCopy(farBuf, farBufReadPos, farend, readPos, readLen);
				// ArrayExtensions.MemCopy(farBuf, farBufReadPos, farend, readPos, readLen);
				farBufReadPos = 0;
				readPos = readLen;
		...
		private void ProcessBlock(short[] farend, short[] nearend, short[] output)
		{
			...
			var df = new ComplexF[AecConfig.PART_LEN1];
            ...
            ArrayExtensions.MemCopy(xBuf, 0, fftBuf, 0, AecConfig.PART_LEN2);
            ArrayExtensions.MemCopy(d, 0, dBuf, AecConfig.PART_LEN, AecConfig.PART_LEN);
            ...
            ArrayExtensions.MemCopy(dBuf, 0, fftBuf, 0, AecConfig.PART_LEN2);
            ...
            ArrayExtensions.MemCopy(xf0, 0, xfBuf0, xfBufBlockPos * AecConfig.PART_LEN1, AecConfig.PART_LEN1);
            ArrayExtensions.MemCopy(xf1, 0, xfBuf1, xfBufBlockPos * AecConfig.PART_LEN1, AecConfig.PART_LEN1);
            ...
            // Error fft
            ArrayExtensions.MemCopy(e, 0, eBuf, AecConfig.PART_LEN, AecConfig.PART_LEN);
            ArrayExtensions.WriteZeroes(fftBuf, 0, AecConfig.PART_LEN);
            ArrayExtensions.MemCopy(e, 0, fftBuf, AecConfig.PART_LEN, AecConfig.PART_LEN);
	...
	for (j = 0; j < WebRtcConstants.PART_LEN1; j++) {
	  ...
	  this.wfBuf[pos + j][1] += MulIm(this.xfBuf[xPos + j][0],
                ...
                ArrayExtensions.WriteZeroes(fftBuf, AecConfig.PART_LEN, AecConfig.PART_LEN);
		...
		{
			ComplexF[] dfw = new ComplexF[AecConfig.PART_LEN1], efw = new ComplexF[AecConfig.PART_LEN1], xfw = new ComplexF[AecConfig.PART_LEN1];
			var comfortNoiseHband = new ComplexF[AecConfig.PART_LEN1];
			var fftBuf = new float[AecConfig.PART_LEN2];
			float dtmp;
			float nlpGainHband;
			int i, j, pos;
			...
			// Buffer far.
			ArrayExtensions.MemCopy(xfw, 0, xfwBuf, 0, xfw.Length);

			// Use delayed far.
			ArrayExtensions.MemCopy(xfwBuf, delayIdx * AecConfig.PART_LEN1, xfw, 0, xfw.Length);
			...
			if (divergeState == 1)
			{
				ArrayExtensions.MemCopy(dfw, 0, efw, 0, efw.Length);
			}
			...
			if (seSum > (19.95f * sdSum))
			{
                ...
                ArrayExtensions.WriteZeroes(wfBuf0, 0, wfBuf0.Length);
                ArrayExtensions.WriteZeroes(wfBuf1, 0, wfBuf1.Length);
			}
			...
			if (hNlXdAvgMin == 1.0f)
			{
				...
				if (stNearState == 1)
				{
                    ArrayExtensions.MemCopy(cohde, 0, hNl, 0, hNl.Length);
					hNlFb = hNlDeAvg;
					hNlFbLow = hNlDeAvg;
				}
				else
			...
			else
			{
				...
				else
				{
                    ...
                    ArrayExtensions.MemCopy(hNl, minPrefBand, hNlPref, 0, prefBandSize);
            ...
            ArrayExtensions.MemCopy(xBuf, AecConfig.PART_LEN, xBuf, 0, AecConfig.PART_LEN);
            ArrayExtensions.MemCopy(dBuf, AecConfig.PART_LEN, dBuf, 0, AecConfig.PART_LEN);
            ArrayExtensions.MemCopy(eBuf, AecConfig.PART_LEN, eBuf, 0, AecConfig.PART_LEN);

			// Copy the current block to the old position for H band
			if (aecConfig.SamplesPerSecond == 32000)
			{
                ArrayExtensions.MemCopy(dBufH, AecConfig.PART_LEN, dBufH, 0, AecConfig.PART_LEN);
			}

			ArrayExtensions.MemCopy(xfwBuf, 0, xfwBuf, AecConfig.PART_LEN1, xfwBuf.Length - AecConfig.PART_LEN1);
		}
		...
		private static void ComfortNoise(ComplexF[] efw, ComplexF[] comfortNoiseHband, float[] noisePow, float[] lambda)
		{
			int i, num;
			var rand = new float[AecConfig.PART_LEN];
			float noiseAvg, tmp, tmpAvg;
			var randW16 = new short[AecConfig.PART_LEN];
			var u = new ComplexF[AecConfig.PART_LEN1];
