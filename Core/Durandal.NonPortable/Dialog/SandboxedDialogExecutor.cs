...
namespace Durandal.Common.Dialog
{
    ...
    public class SandboxedDialogExecutor : AbstractDialogExecutor
    {
        ...
        public override async Task<DialogProcessingResponse> LaunchPlugin(DurandalPlugin plugin, string entryPointName, bool isRetry, QueryWithContext argument, IPluginServices services, ILogger queryLogger)
        {
            DialogResult pluginResult = null;
            ...
            if (entryPoint == null)
            {
                pluginResult = new DialogResult(Result.Failure);
            ...
            Thread remoteExecutionThread = new Thread(() =>
            {
                ...
                catch (SecurityException e)
                {
                    ...
                    pluginResult.ErrorMessage = errorMessage;
                    queryLogger.Log(errorMessage, LogLevel.Err);
                    queryLogger.Log(e.ToString(), LogLevel.Err);
                    pluginException = e;
                }
                catch (Exception e)
                {
        ...
        public override async Task<TriggerProcessingResponse> TriggerPlugin(DurandalPlugin plugin, QueryWithContext query, IPluginServices services, ILogger queryLogger)
        {
        ...
        public override async Task<bool> LoadPlugin(DurandalPlugin plugin, IPluginServices localServices, ILogger logger)
        {
            try
            {
                await base.LoadPlugin(plugin, localServices, logger);
                await plugin.OnLoad(localServices);
                return true;
            }
            catch (Exception e)
            {
        ...
        public override async Task<bool> UnloadPlugin(DurandalPlugin plugin, IPluginServices localServices, ILogger logger)
        {
            try
            {
                await base.UnloadPlugin(plugin, localServices, logger);
                await plugin.OnUnload(localServices);
                return true;
            }
            catch (Exception e)
            {
        ...
        public override Task<CrossDomainResponseData> CrossDomainResponse(DurandalPlugin plugin, CrossDomainContext context)
        {
            return plugin.CrossDomainResponse(context);
        }
    }
}
