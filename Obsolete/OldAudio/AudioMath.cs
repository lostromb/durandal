...
namespace Durandal.Common.Audio
{
    public static class AudioMath
    {
        ...
        public static void ShortsToFloats(short[] input, int in_offset, float[] output, int out_offset, int samples)
        {
            for (int c = 0; c < samples; c++)
            {
                output[c + out_offset] = (float)input[c + in_offset] / (float)(short.MaxValue);
            }
        }

        public static void FloatsToShorts(float[] input, int in_offset, short[] output, int out_offset, int samples, bool clamp = true)
        {
            if (clamp)
            {
                for (int c = 0; c < samples; c++)
                {
                    float sample = input[c + in_offset] * short.MaxValue;
            ...
            else
            {
                for (int c = 0; c < samples; c++)
                {
                    output[c + out_offset] = (short)(input[c + in_offset] * short.MaxValue);
                }
            }
        }
