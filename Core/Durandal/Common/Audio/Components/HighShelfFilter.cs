...
{
    public sealed class HighShelfFilter : BiquadFilter
    {
        private float _gainDb;
        private float _frequency;

        /// <summary>
        /// Constructs a new high shelf filter (highpass without completely muting the lower bands)
        /// </summary>
        ...
        /// <param name="gainDB">The filter's gain value in dB, or in other words, the amount that lower bands will be muted.</param>
        public HighShelfFilter(WeakPointer<IAudioGraph> graph, AudioSampleFormat format, string nodeCustomName, float frequencyHz, float gainDB)
            : base(graph, format, nameof(HighShelfFilter), nodeCustomName)
        {
            if (frequencyHz < 0)
            {
        ...
        protected override BiquadCoefficients CalculateBiQuadCoefficients()
        {
            ...
            if (_gainDb >= 0)
            {
                ...
                A0 = (v + (float)Math.Sqrt(2 * v) * k + k * k) * norm;
                A1 = 2 * (k * k - v) * norm;
                A2 = (v - (float)Math.Sqrt(2 * v) * k + k * k) * norm;
                B1 = 2 * (k * k - 1) * norm;
            ...
            {
                // cut
                norm = 1 / (v + (float)Math.Sqrt(2 * v) * k + k * k);
                A0 = (1 + sqrt2 * k + k * k) * norm;
                A1 = 2 * (k * k - 1) * norm;
                A2 = (1 - sqrt2 * k + k * k) * norm;
                B1 = 2 * (k * k - v) * norm;
                B2 = (v - (float)Math.Sqrt(2 * v) * k + k * k) * norm;
            }
