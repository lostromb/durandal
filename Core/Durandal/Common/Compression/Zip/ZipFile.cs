...
namespace Durandal.Common.Compression.Zip
{
    ...
    IDisposable
    {
        ...
        ///
        /// <para>
        ///   This flag is effective only when calling Initialize(). Normally you would read a ZipFile with the
        ///   static ZipFile.Read
        ...
        ///   information see <see
        ///   cref="Durandal.Common.Compression.ZLib.CompressionStrategy">ZLib.CompressionStrategy</see>.
        /// </remarks>
        public Durandal.Common.Compression.ZLib.CompressionStrategy Strategy
        {
        ...
        ///   calling ZipFile.Save(). When instantiating a
        ...
        ///   </para>
        /// </remarks>
        /// <seealso cref="Durandal.Common.Compression.Zip.CompressionMethod" />
        public Durandal.Common.Compression.Zip.CompressionMethod CompressionMethod
        {
            get
            {
        ...
        public bool UseUnicodeAsNecessary
        {
            ...
            set
            {
                ...
                {
                    _alternateEncoding = Durandal.Common.Compression.Zip.ZipFile.DefaultEncoding;
        ...
        /// </example>
        ///
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.DefaultEncoding">DefaultEncoding</seealso>
        ...
        /// </exception>
        ///
        public VirtualPath TempFileFolder
        {
        ...
        /// </example>
        ///
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipEntry.ZipErrorAction"/>
        ...
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipEntry.Encryption">ZipEntry.Encryption</seealso>
        ...
        /// <remarks>
        ///   <para>
        ///     Set this to a non-zero value before calling Save()
        ...
        ///
        ///   <para>
        ///     If you read a split archive, with ZipFile.Read() and
        ...
        internal Stream StreamForDiskNumber(uint diskNumber)
        {
            ...
            return ZipSegmentedStream.ForReading(this._readName ?? this._name, _fileSystem, _logger,
        ...
        internal void Reset(bool whileSaving)
        {
            if (_JustSaved)
            {
                ...
                using (ZipFile x = new ZipFile(_logger, _fileSystem))
                {
                    if (_fileSystem.Exists(this._readName ?? this._name))
                    {
                        // workitem 10735
        ...
        ///   better form to use the static ZipFile.Read() method,
        ...
        /// </remarks>
        ///
        /// <exception cref="Durandal.Common.Compression.Zip.ZipException">
        ...
        ///
        public ZipFile(VirtualPath fileName, ILogger logger, IFileSystem fileSystem)
        {
            ...
            try
            {
                _InitInstance(fileName, null, logger, fileSystem);
            }
            catch (Exception e1)
            {
        ...
        public ZipFile(VirtualPath fileName, ILogger logger, IFileSystem fileSystem, System.Text.Encoding encoding)
        {
            try
            {
        ...
        ///   archive, the application should call ZipFile.Save() or
        ...
        /// <param name="logger">A logger</param>
        /// <param name="fileSystem">The file system for accessing files</param>
        public ZipFile(ILogger logger, IFileSystem fileSystem)
        {
            if (DefaultEncoding == null)
            {
                _alternateEncoding = System.Text.Encoding.UTF8;
            ...
            _InitInstance(null, null, logger, fileSystem);
        }
        ...
        /// filenames or comments that cannot be encoded with the IBM437 code page.
        /// </param>
        public ZipFile(ILogger logger, IFileSystem fileSystem, System.Text.Encoding encoding)
        {
            AlternateEncoding = encoding;
            AlternateEncodingUsage = ZipOption.Always;
            _InitInstance(null, null, logger, fileSystem);
        ...
        /// <remarks>
        /// <para>
        ///   See the documentation on theZipFile
        ///   constructor that accepts a single string argument for basic
        ///   information on all the <c>ZipFile</c> constructors.
        /// </para>
        ///
        ...
        public ZipFile(VirtualPath fileName, ILogger logger, IFileSystem fileSystem, TextWriter statusMessageWriter)
        {
            if (DefaultEncoding == null)
            {
                _alternateEncoding = System.Text.Encoding.UTF8;
            ...
            try
            {
                _InitInstance(fileName, statusMessageWriter, logger, fileSystem);
            }
            catch (Exception e1)
            {
                throw new ZipException(String.Format("{0} is not a valid zip file", fileName), e1);
        ...
        ///   constructor, then calling <c>Initialize()</c> to read the contents
        ...
        /// <param name="logger">A logger</param>
        /// <param name="fileSystem">The file system for accessing files</param>
        public void Initialize(VirtualPath fileName, ILogger logger, IFileSystem fileSystem)
        {
            try
            {
                _InitInstance(fileName, null, logger, fileSystem);
        ...
        private void _InitInstance(VirtualPath zipFileName, TextWriter statusMessageWriter, ILogger logger, IFileSystem fileSystem)
        {
            ...
            CompressionLevel = Durandal.Common.Compression.ZLib.CompressionLevel.Default;
            ...
            if (_name != null && _fileSystem.Exists(_name))
            {
                if (FullScan)
        ...
        /// </code>
        /// </example>
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.RemoveEntry(string)"/>
        ///
        ...
        /// </remarks>
        ///
        /// <seealso cref="Durandal.Common.Compression.Zip.ZipFile.this[string]"/>
        ///
        /// <example>
        ...
        internal Stream ReadStream
        {
            get
            {
                if (_readstream == null)
                {
                    ...
                    {
                        _readstream = _fileSystem.OpenStream(_readName ?? _name, FileOpenMode.Open, FileAccessMode.Read);
        ...
        private Stream WriteStream
        {
            ...
            get
            {
                ...
                {
                    _writestream = ZipSegmentedStream.ForWriting(this._name, _fileSystem, _logger, _maxOutputSegmentSize);
                    return _writestream;
                }

                SharedUtilities.CreateAndOpenUniqueTempFile(TempFileFolder ?? _name.Container,
                    _fileSystem,
                    out _writestream,
        ...
        private VirtualPath _name;
        private VirtualPath _readName;
        private string _Comment;
        internal string _Password;
        private bool _emitNtfsTimes = true;
        private bool _emitUnixTimes;
        private Durandal.Common.Compression.ZLib.CompressionStrategy _Strategy = Durandal.Common.Compression.ZLib.CompressionStrategy.Default;
        private Durandal.Common.Compression.Zip.CompressionMethod _compressionMethod = Durandal.Common.Compression.Zip.CompressionMethod.Deflate;
        private bool _fileAlreadyExists;
        private VirtualPath _temporaryFileName;
        private bool _contentsChanged;
        private bool _hasBeenSaved;
        private VirtualPath _TempFileFolder;
